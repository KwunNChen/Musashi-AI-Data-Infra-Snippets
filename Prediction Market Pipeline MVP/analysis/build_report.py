...
body(
    ...
    "on an unattended GitHub Actions cron job, nominally every 2 hours. On the analysis side, I used the collected "
...
body("A GitHub Actions workflow (<b>.github/workflows/prediction-markets-w1.yml</b>, at the repo root, since "
     ...
     "2 hours, or tries to, see below. It's offset from the top of the hour to avoid the queueing delays GitHub applies when many "
...
body("<b>The requested schedule is not the delivered schedule.</b> Measuring the actual arrival times of 41 "
     "poll batches over 200 hours, the median gap between batches is 4.6 hours, not 2, with a worst observed "
     "gap of 34.6 hours. GitHub delivered roughly 41% of the scheduled runs (4.9 batches per day against 12 "
     "requested). Every batch that does land is complete, writing all 19 markets, so this is GitHub dropping or "
     "delaying scheduled events under load rather than the ingestion failing. The workflow reports green "
     "throughout, because the runs that execute genuinely succeed. Anything depending on a guaranteed interval "
     "would need a scheduler that isn't GitHub Actions cron, such as Supabase pg_cron, which was the alternative "
